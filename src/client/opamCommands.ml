...
let switch =
  ...
  let switch
      ...
      no_switch packages formula empty descr full no_install deps_only repos
      force no_action
  ...
  Term.(ret (const switch
             ...
             $print_short_flag
             $no_switch
             $packages $formula $empty $descr $full $no_install $deps_only
             $repos $force $no_action $d_alias_of $d_no_autoinstall $params)),
