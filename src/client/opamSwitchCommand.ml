...
let guess_compiler_invariant ?repos rt strings =
  (* TODO: allow version constraints *)
  let repos = match repos with
    | None -> OpamGlobalState.repos_list rt.repos_global
    | Some r -> r
  in
   ...
   *   OpamPackage.Map.filter (fun nv opam ->
   *       OpamFile.OPAM.has_flag Pkgflag_Compiler opam &&
   *       OpamFilter.eval_to_bool ~default:false
   *         (OpamPackageVar.resolve_global rt.repos_global)
   *         (OpamFile.OPAM.available opam))
  ...
  let invariant =
    List.fold_left (fun acc str ->
        match OpamFormula.atom_of_string str with
        | name, _ as atom ->
          ...
          else no_compiler_error str
        | exception Failure _ ->
          match OpamPackage.Version.of_string str with
          | v ->
            let candidates =
   ...
   *  *     | n, None -> OpamPackage.has_name compiler_packages n
   *  *     | n, Some (_, v) -> OpamPackage.(Set.mem (create n v) compiler_packages))
   *  *     atoms
   *  * then
   *  *   invariant
   *  * else *\)
   * let dep_comps =
   *   (\* Look for compilers that are common dependencies of the selected
   *      packages *\)
   *   OpamPackage.Set.fold (fun nv acc ->
   *       let opam = OpamPackage.Map.find nv opams in
   *       let env =
   *         OpamPackageVar.resolve_switch_raw ~package:nv rt.repos_global
   *           (OpamSwitch.of_string "unnamed")
   *           (OpamFile.Switch_config.empty)
   *       in
   *       OpamFormula.packages acc @@
   *       OpamPackageVar.filter_depends_formula ~env
   *         ~default:false ~test:false ~dev:false ~post:true
   *         (OpamFile.OPAM.depends opam))
   *     (OpamFormula.packages packages invariant)
   *     packages
   * in
   * if OpamPackage.Set.is_empty dep_comps then
   *   no_compiler_error (OpamFormula.to_string invariant)
   * else
   *   OpamPackage.Name.Map.fold (fun n vs acc ->
   *       let allvs = OpamPackage.versions_of_name packages n in
   *       OpamFormula.ands [
   *         acc;
   *         Atom (n, OpamFormula.formula_of_version_set allvs vs)
   *       ])
   *     (OpamPackage.to_map dep_comps)
   *     OpamFormula.Empty *)

(*
  let st =
    OpamSwitchState.load_virtual ~repos_list:repos ~avail_default:false
      rt.repos_global rt
  in
  let universe =
    let requested =
    ...
    OpamSwitchState.universe
      { st with switch_invariant = OpamFormula.of_atom_formula invariant }
      ~requested Switch
  in
  log "Calling solver to help choose a compiler";
  match OpamSolver.resolve universe ~orphans:OpamPackage.Set.empty
          (OpamSolver.request ()) with
  | Conflicts cs ->
      OpamConsole.error_and_exit `No_solution
        "No solution found for %s: %s"
        (OpamFormula.string_of_atoms atoms)
        (OpamCudf.string_of_conflict st.packages
           (OpamSwitchState.unavailable_reason st) cs)
  | Success s ->
    let installed = OpamSolver.new_packages s in
    log "Soluion: %s" (OpamPackage.Set.to_string installed);
    let comp = OpamPackage.Set.inter compiler_packages installed in
    let roots = OpamFormula.packages_of_atoms installed atoms in
    let invariant =
      OpamSwitchState.infer_switch_invariant
        { st with
          installed;
          compiler_packages = comp;
          installed_roots = roots; }
    in
    OpamConsole.note "Opam didn't like your request, so the following switch \
                      invariant will be selected instead:\n%s"
      (OpamFormula.to_string invariant);
    invariant
*)

let guess_compiler_invariant ?repos rt strings =
  let r = guess_compiler_invariant ?repos rt strings in
  OpamConsole.log "SW" "Inferred switch invariant: %s"
    (OpamFileTools.dep_formula_to_string r);
  r

(*
  let r = OpamListCommand

  
  match atoms with
  | [] 
  match List.map OpamPackage.of_string_opt strings with
  | [Some nv] when OpamPackage.Set.mem nv compiler_packages ->
    OpamFormula.Atom (OpamSolution.eq_atom_of_package nv)
